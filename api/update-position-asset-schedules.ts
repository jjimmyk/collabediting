...
import { replacePositionAssetSchedules } from './roster-asset-assignments-shared.js'
...
function formatHandlerError(error: unknown): string {
  ...
  return 'Update position asset schedules failed.'
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const assignAssetKeys = Array.isArray(body.assignAssetKeys)
      ? body.assignAssetKeys.filter((entry): entry is string => typeof entry === 'string')
      : []
    const unassignAssetKeys = Array.isArray(body.unassignAssetKeys)
      ? body.unassignAssetKeys.filter((entry): entry is string => typeof entry === 'string')
      : []
    ...
    const result = await replacePositionAssetSchedules(admin, {
      workspaceId,
      positionName,
      assignAssetKeys,
      unassignAssetKeys,

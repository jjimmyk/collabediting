...
import {
  addNotificationRuleCondition,
  createNotificationCreationRule,
  deriveNotificationCreationRuleLogic,
  NOTIFICATION_DATA_INPUTS,
  NOTIFICATION_THRESHOLD_OPERATORS,
  formatNotificationCreationRuleFormula,
  formatNotificationCreationRuleNaturalLanguageSummary,
  getNotificationCreationRuleSearchText,
  getNotificationRuleConditionById,
  normalizeNotificationCreationRule,
  removeNotificationRuleCondition,
  updateNotificationRuleCondition,
  updateNotificationRuleGroupInternalJoin,
  updateNotificationRuleLogicBlockJoin,
  type AddNotificationRuleConditionMode,
  type NotificationConditionJoin,
  type NotificationCreationRule,
  type NotificationRuleLogicBlock,
...
type NotificationSettingsPageProps = {
  rules: NotificationCreationRule[]
  onRulesChange: Dispatch<SetStateAction<NotificationCreationRule[]>>
  categoryOptions: string[]
  onGenerateRule: () => void
}

const cloneNotificationCreationRule = (rule: NotificationCreationRule): NotificationCreationRule =>
  structuredClone(normalizeNotificationCreationRule(rule))
...
export function NotificationSettingsPage({
  ...
  const [ruleDrafts, setRuleDrafts] = useState<Record<number, NotificationCreationRule>>({})
  ...
  const filteredRules = useMemo(
    () =>
      normalizedRuleSearchQuery
        ? rules.filter((rule) => getNotificationCreationRuleSearchText(rule).includes(normalizedRuleSearchQuery))
        : rules,
  ...
  const updateRuleDraft = (
    ruleId: number,
    updater: (rule: NotificationCreationRule) => NotificationCreationRule
  ) => {
    setRuleDrafts((previous) => {
      const current =
        previous[ruleId] ?? rules.find((rule) => rule.id === ruleId) ?? createNotificationCreationRule(rules)
      return {
        ...previous,
  ...
  const startEditingRule = (rule: NotificationCreationRule) => {
    setPendingAddCondition(null)
    setRuleDrafts((previous) => ({
      ...previous,
      [rule.id]: cloneNotificationCreationRule(rule),
    }))
  ...
  const saveRuleDraft = (ruleId: number) => {
    ...
    onRulesChange((previous) =>
      previous.map((rule) =>
        rule.id === ruleId
          ? cloneNotificationCreationRule(
              normalizeNotificationCreationRule({
                ...draft,
                logic: deriveNotificationCreationRuleLogic(draft),
              })
            )
          : rule
      )
  ...
  useEffect(() => {
    ...
    const element = document.getElementById(`notification-rule-${scrollToRuleId}`)
    if (!element) {
      return
    }
  ...
  const toggleNotificationUser = (rule: NotificationCreationRule, userId: string) => {
  ...
  const toggleNotificationGroup = (rule: NotificationCreationRule, groupId: string) => {
    updateRuleDraft(rule.id, (current) => ({
      ...current,
  ...
  const renderNotificationSettingsEditor = (rule: NotificationCreationRule) => (
    <div className="space-y-3 rounded-md border bg-muted/10 p-3">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-sm font-medium">Notification delivery</p>
          <p className="text-xs text-muted-foreground">
            Notify selected users and groups when this rule auto-sends a notification.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Switch
            id={`notification-rule-send-notification-${rule.id}`}
          ...
          />
          <Label htmlFor={`notification-rule-send-notification-${rule.id}`}>Send notification</Label>
        </div>
      </div>
  ...
  const renderNotificationSettingsReadOnly = (rule: NotificationCreationRule) => (
  ...
  const addConditionToRule = (
    ...
    updateRuleDraft(ruleId, (current) => addNotificationRuleCondition(current, { mode, join }))
    setPendingAddCondition(null)
  }

  const renderJoinSelector = (
    value: NotificationConditionJoin,
    onChange: (join: NotificationConditionJoin) => void,
    label?: string
  ) => (
  ...
  const renderConditionEditor = (
    ...
    const selectedInput = NOTIFICATION_DATA_INPUTS.find((input) => input.id === condition.dataInputId)
      ...
      >
        <div className="grid gap-1.5">
          ...
          <Select
            ...
            onValueChange={(value) =>
              updateRuleDraft(rule.id, (current) =>
                updateNotificationRuleCondition(current, conditionId, (entry) => ({
                  ...entry,
                  dataInputId: value,
                }))
              )
          ...
          >
            ...
            <SelectContent>
              {NOTIFICATION_DATA_INPUTS.map((input) => (
        ...
        <div className="grid gap-1.5">
          ...
          >
            ...
            <SelectContent>
              {NOTIFICATION_THRESHOLD_OPERATORS.map((operator) => (
        ...
        <div className="flex items-end justify-end">
          <Button
            ...
            disabled={!canRemove}
            onClick={() =>
              updateRuleDraft(rule.id, (current) => removeNotificationRuleCondition(current, conditionId))
            }
          >
  ...
  const renderLogicGroupBlock = (
    rule: NotificationCreationRule,
    block: Extract<NotificationRuleLogicBlock, { type: 'group' }>
  ) => (
          ...
          <div key={conditionId} className="relative space-y-2">
            ...
            {conditionIndex > 0 &&
              renderJoinSelector(block.internalJoin, (join) =>
                updateRuleDraft(rule.id, (current) =>
                  updateNotificationRuleGroupInternalJoin(current, block.id, join)
                )
              )}
  ...
  const renderLogicBlock = (
    rule: NotificationCreationRule,
    block: NotificationRuleLogicBlock,
    blockIndex: number
  ) => {
    if (block.type === 'group') {
  ...
  const renderAddConditionPanel = (rule: NotificationCreationRule) => {
  ...
  const renderRuleEditor = (rule: NotificationCreationRule) => {
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-3">
        <div className="grid min-w-[14rem] flex-1 gap-1.5">
          <Label htmlFor={`notification-rule-name-${rule.id}`}>Rule name</Label>
          <Input
            id={`notification-rule-name-${rule.id}`}
            value={rule.name}
            onChange={(event) =>
              updateRuleDraft(rule.id, (current) => ({
                ...current,
        ...
        <div className="flex items-center gap-2 pt-6">
          <Switch
            id={`notification-rule-enabled-${rule.id}`}
          ...
          />
          <Label htmlFor={`notification-rule-enabled-${rule.id}`}>Enabled</Label>
        </div>
      </div>

      <div className="rounded-md border bg-background/80 p-3">
        <p className="text-sm font-medium">Logic / formula builder</p>
        <p className="mt-3 text-sm font-medium">Send a Notification when</p>
            ...
            <div key={block.type === 'group' ? `group-${block.id}` : `condition-${block.conditionId}`}>
              {blockIndex > 0 &&
                renderJoinSelector(block.joinWithPrevious ?? 'and', (join) =>
                  updateRuleDraft(rule.id, (current) =>
                    updateNotificationRuleLogicBlockJoin(current, blockIndex, join)
                  )
                )}
        ...
        <div className="mt-3 rounded-md border border-dashed bg-background px-3 py-2">
          ...
          <p className="mt-1 text-sm">
            ...
            <span className="font-medium">{formatNotificationCreationRuleFormula(normalizedRule)}</span>
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <div className="grid gap-1.5">
          <Label htmlFor={`notification-rule-template-${rule.id}`}>Notification title template</Label>
          <Input
            id={`notification-rule-template-${rule.id}`}
            value={rule.notificationTitleTemplate}
            onChange={(event) =>
              updateRuleDraft(rule.id, (current) => ({
                ...current,
                notificationTitleTemplate: event.target.value,
              }))
            }
            placeholder="Threshold Notification — {category}"
          />
        </div>
        <div className="grid gap-1.5">
          <Label>Category</Label>
          <Select
            value={rule.category || undefined}
            onValueChange={(value) =>
              updateRuleDraft(rule.id, (current) => ({
                ...current,
                category: value,
        ...
        <div className="grid gap-1.5">
          ...
          <Select
            value={rule.severity}
            onValueChange={(value: NotificationCreationRule['severity']) =>
              updateRuleDraft(rule.id, (current) => ({
                ...current,
                severity: value,
              }))
        ...
        <div className="grid gap-1.5 md:col-span-2">
          <Label htmlFor={`notification-rule-threshold-description-${rule.id}`}>
            Threshold description
          </Label>
          <Textarea
            id={`notification-rule-threshold-description-${rule.id}`}
            ...
            placeholder="Shown on auto-sent notifications when this rule fires."
  ...
  const renderRuleReadOnly = (rule: NotificationCreationRule) => (
    <div className="space-y-3">
      <div className="grid gap-2 text-sm md:grid-cols-2">
        ...
        <p className="md:col-span-2">
          <span className="font-medium">Formula:</span> {formatNotificationCreationRuleFormula(rule)}
        </p>
        <p>
          <span className="font-medium">Notification title template:</span> {rule.notificationTitleTemplate}
        </p>
        <p>
          <span className="font-medium">Category:</span>{' '}
          {rule.category || 'Not set'}
        </p>
        <p>
    ...
    <div className="flex h-full min-h-0 flex-col gap-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <p className="max-w-2xl text-sm text-muted-foreground">
          Configure logic that auto-sends notifications when monitored data inputs cross defined
          thresholds.
        </p>
        <div className="flex flex-wrap gap-2">
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <p className="text-sm font-medium">Automated Notification Rules</p>
          <div className="flex h-8 w-full max-w-sm items-center gap-2 rounded-md border px-2 text-xs">
            ...
            <input
              ...
              placeholder="Search rules by name, data input, category, or logic"
              aria-label="Search automated notification rules"
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-y-auto pr-1 [scrollbar-gutter:stable]">
          {rules.length === 0 ? (
            <div className="rounded-md border border-dashed px-4 py-8 text-center text-sm text-muted-foreground">
              No notification creation rules yet. Add a rule or use Generate Rule to start building threshold
              logic.
            </div>
            ...
            filteredRules.map((rule) => {
              ...
              const summary = formatNotificationCreationRuleNaturalLanguageSummary(rule)

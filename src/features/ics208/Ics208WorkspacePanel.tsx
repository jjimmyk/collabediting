...
import { Ics208ExportPreviewDialog } from '@/features/ics208/Ics208ExportPreviewDialog'
import { Ics208FormSections } from '@/features/ics208/Ics208FormSections'
import {
  buildIcs208DocxBlocks,
  buildIcs208ExportOptions,
  ics208ExportFilenameBase,
  type Ics208DocxBlock,
} from '@/features/ics208/export'
import type {
...
import { cloneIcs208FormState, getIcs208FormForExport } from '@/features/ics208/utils'
import {
...
type ExportBlock = Ics208DocxBlock
type ExportOptions = ReturnType<typeof buildIcs208ExportOptions>

type Ics208WorkspacePanelProps = {
  form: Ics208FormState | null
  setForm: React.Dispatch<React.SetStateAction<Ics208FormState | null>>
  ...
  onPatchSectionDraft: <S extends Ics208SectionId>(
    section: S,
...
export function Ics208WorkspacePanel({
  form,
  setForm,
  versions,
  canEdit,
  ...
  downloadDocx,
  downloadPdf,
}: Ics208WorkspacePanelProps) {
  const liveFormRef = useRef<Ics208FormState | null>(null)
  const [viewingPastVersion, setViewingPastVersion] = useState<Ics208Version | null>(null)
  ...
  const [previewBlocks, setPreviewBlocks] = useState<Ics208DocxBlock[]>([])
  ...
  const getExportForm = () => {
    if (!form) return null
    return getIcs208FormForExport(form, sectionDrafts)
  }
  ...
  const openPreview = () => {
    ...
    setPreviewBlocks(buildIcs208DocxBlocks(exportForm, getExportContext()))
  ...
  const exportWord = (blocks?: ExportBlock[]) => {
    ...
    downloadDocx(
      `ICS-208_${ics208ExportFilenameBase(exportForm)}_${stamp}.docx`,
      blocks ?? buildIcs208DocxBlocks(exportForm, context),
      buildIcs208ExportOptions(exportForm, context)
    )
  }
  ...
  const exportPdf = (blocks?: ExportBlock[]) => {
    ...
    const stamp = new Date().toISOString().slice(0, 16).replace(/[:T]/g, '-')
    downloadPdf(
      `ICS-208_${ics208ExportFilenameBase(exportForm)}_${stamp}.pdf`,
      blocks ?? buildIcs208DocxBlocks(exportForm, context),
      buildIcs208ExportOptions(exportForm, context)
    )
  }
  ...
  const viewVersion = (version: Ics208Version) => {
    if (!form) return
    if (!viewingPastVersion) {
      liveFormRef.current = cloneIcs208FormState(form)
    }
    setForm(cloneIcs208FormState(version.snapshot))
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-end gap-2">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              <DropdownMenuLabel>Export ICS-208</DropdownMenuLabel>
              <DropdownMenuSeparator />
      ...
      >
        <DialogContent className="!w-[60vw] !max-w-[60vw] sm:!max-w-[60vw]">
          <div className="flex items-center gap-2 px-1 pb-2 text-sm font-semibold">
            ...
            {versionDialogKind === 'signed' ? 'Signed versions' : 'Version history'} — ICS-208
          </div>
                ...
                {[...shownVersions].reverse().map((version) => {
                  ...
                  const versionLabel =
                    indexInAll === versions.length - 1 && versionDialogKind === 'all'
                      ...
                      : `v${indexInAll + 1}`
                  const preview = (
                    version.snapshot.safetyMessagePlan.slice(0, 80) ||
      ...
      <Ics208ExportPreviewDialog
        open={isPreviewOpen}
        onOpenChange={setIsPreviewOpen}
        title={`ICS-208 Preview — ${form.incidentName || incidentName || 'Safety & Health Plan'}`}

...
import { Ics208hmExportPreviewDialog } from '@/features/ics208hm/Ics208hmExportPreviewDialog'
import { Ics208hmFormSections } from '@/features/ics208hm/Ics208hmFormSections'
import {
  buildIcs208hmDocxBlocks,
  buildIcs208hmExportOptions,
  ics208hmExportFilenameBase,
  type Ics208hmDocxBlock,
} from '@/features/ics208hm/export'
import type {
...
import { cloneIcs208hmFormState, getIcs208hmFormForExport } from '@/features/ics208hm/utils'
import { cn } from '@/lib/utils'

type ExportBlock = Ics208hmDocxBlock
type ExportOptions = ReturnType<typeof buildIcs208hmExportOptions>

type Ics208hmWorkspacePanelProps = {
  form: Ics208hmFormState | null
  setForm: React.Dispatch<React.SetStateAction<Ics208hmFormState | null>>
  ...
  onPatchSectionDraft: <S extends Ics208hmSectionId>(
    section: S,
...
export function Ics208hmWorkspacePanel({
  form,
  setForm,
  versions,
  canEdit,
  ...
  downloadDocx,
  downloadPdf,
}: Ics208hmWorkspacePanelProps) {
  const liveFormRef = useRef<Ics208hmFormState | null>(null)
  const [viewingPastVersion, setViewingPastVersion] = useState<Ics208hmVersion | null>(null)
  ...
  const [previewBlocks, setPreviewBlocks] = useState<Ics208hmDocxBlock[]>([])
  ...
  const getExportForm = () => {
    if (!form) return null
    return getIcs208hmFormForExport(form, sectionDrafts)
  }
  ...
  const openPreview = () => {
    ...
    setPreviewBlocks(buildIcs208hmDocxBlocks(exportForm, getExportContext()))
  ...
  const exportWord = (blocks?: ExportBlock[]) => {
    ...
    downloadDocx(
      `ICS-208HM_${ics208hmExportFilenameBase(exportForm)}_${stamp}.docx`,
      blocks ?? buildIcs208hmDocxBlocks(exportForm, context),
      buildIcs208hmExportOptions(exportForm, context)
    )
  }
  ...
  const exportPdf = (blocks?: ExportBlock[]) => {
    ...
    const stamp = new Date().toISOString().slice(0, 16).replace(/[:T]/g, '-')
    downloadPdf(
      `ICS-208HM_${ics208hmExportFilenameBase(exportForm)}_${stamp}.pdf`,
      blocks ?? buildIcs208hmDocxBlocks(exportForm, context),
      buildIcs208hmExportOptions(exportForm, context)
    )
  }
  ...
  const viewVersion = (version: Ics208hmVersion) => {
    if (!form) return
    if (!viewingPastVersion) {
      liveFormRef.current = cloneIcs208hmFormState(form)
    }
    setForm(cloneIcs208hmFormState(version.snapshot))
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-end gap-2">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              <DropdownMenuLabel>Export ICS-208HM</DropdownMenuLabel>
              <DropdownMenuSeparator />
        ...
        {isCreatingSignedVersion && !viewingPastVersion && (
          <div className="flex items-center justify-end rounded-md border border-sky-400 bg-sky-50/60 px-3 py-2 dark:border-sky-500 dark:bg-sky-500/10">
            <Button
              ...
              onClick={() => {
                setSignNameInput('You')
                setSignDialog({ mode: 'new-version', role: 'HM Group Supervisor' })
              }}
            >
              <Check className="h-3.5 w-3.5" />
              Sign this version
      ...
      >
        <DialogContent className="!w-[60vw] !max-w-[60vw] sm:!max-w-[60vw]">
          <div className="flex items-center gap-2 px-1 pb-2 text-sm font-semibold">
            ...
            {versionDialogKind === 'signed' ? 'Signed versions' : 'Version history'} — ICS-208HM
          </div>
                ...
                {[...shownVersions].reverse().map((version) => {
                  ...
                  const versionLabel =
                    indexInAll === versions.length - 1 && versionDialogKind === 'all'
                      ...
                      : `v${indexInAll + 1}`
                  const preview = (
                    version.snapshot.entryObjectives.slice(0, 80) ||
                    version.snapshot.organization.hmGroupSupervisor ||
      ...
      <Ics208hmExportPreviewDialog
        open={isPreviewOpen}
        onOpenChange={setIsPreviewOpen}
        title={`ICS-208HM Preview — ${form.incidentName || incidentName || 'Safety & Health Plan'}`}

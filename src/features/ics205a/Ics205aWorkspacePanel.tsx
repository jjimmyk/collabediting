...
import { Ics205aExportPreviewDialog } from '@/features/ics205a/Ics205aExportPreviewDialog'
import { Ics205aFormSections } from '@/features/ics205a/Ics205aFormSections'
import {
  buildIcs205aDocxBlocks,
  buildIcs205aExportOptions,
  ics205aExportFilenameBase,
  type Ics205aDocxBlock,
} from '@/features/ics205a/export'
import type {
...
type ExportBlock = Ics205aDocxBlock
type ExportOptions = ReturnType<typeof buildIcs205aExportOptions>

type Ics205aWorkspacePanelProps = {
  form: Ics205aFormState | null
  setForm: React.Dispatch<React.SetStateAction<Ics205aFormState | null>>
  ...
  onPatchSectionDraft: <S extends Ics205aSectionId>(
    section: S,
...
export function Ics205aWorkspacePanel({
  form,
  setForm,
  versions,
  canEdit,
  ...
  const liveFormRef = useRef<Ics205aFormState | null>(null)
  const [viewingPastVersion, setViewingPastVersion] = useState<Ics205aVersion | null>(null)
  ...
  const [previewBlocks, setPreviewBlocks] = useState<Ics205aDocxBlock[]>([])
  ...
  const openPreview = () => {
    ...
    setPreviewBlocks(buildIcs205aDocxBlocks(exportForm, getExportContext()))
  ...
  const exportWord = (blocks?: ExportBlock[]) => {
    ...
    downloadDocx(
      `ICS-205A_${ics205aExportFilenameBase(exportForm)}_${stamp}.docx`,
      blocks ?? buildIcs205aDocxBlocks(exportForm, context),
      buildIcs205aExportOptions(exportForm, context)
    )
  }
  ...
  const exportPdf = (blocks?: ExportBlock[]) => {
    ...
    const stamp = new Date().toISOString().slice(0, 16).replace(/[:T]/g, '-')
    downloadPdf(
      `ICS-205A_${ics205aExportFilenameBase(exportForm)}_${stamp}.pdf`,
      blocks ?? buildIcs205aDocxBlocks(exportForm, context),
      buildIcs205aExportOptions(exportForm, context)
    )
  }
  ...
  const viewVersion = (version: Ics205aVersion) => {
    if (!form) return
    if (!viewingPastVersion) {
      liveFormRef.current = cloneIcs205aFormState(form)
    }
    setForm(cloneIcs205aFormState(version.snapshot))
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-end gap-2">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              <DropdownMenuLabel>Export ICS-205A</DropdownMenuLabel>
              <DropdownMenuSeparator />
      ...
      >
        <DialogContent className="!w-[60vw] !max-w-[60vw] sm:!max-w-[60vw]">
          <div className="flex items-center gap-2 px-1 pb-2 text-sm font-semibold">
            ...
            {versionDialogKind === 'signed' ? 'Signed versions' : 'Version history'} — ICS-205A
          </div>
                ...
                {[...shownVersions].reverse().map((version) => {
                  ...
                  const versionLabel =
                    indexInAll === versions.length - 1 && versionDialogKind === 'all'
                      ...
                      : `v${indexInAll + 1}`
                  const preview = (
                    version.snapshot.contactRows.find((row) => row.name.trim())?.name ||
      ...
      <Ics205aExportPreviewDialog
        open={isPreviewOpen}
        onOpenChange={setIsPreviewOpen}
        title={`ICS-205A Preview — ${form.incidentName || incidentName || 'Communications List'}`}

...
import { Ics205ExportPreviewDialog } from '@/features/ics205/Ics205ExportPreviewDialog'
import { Ics205FormSections } from '@/features/ics205/Ics205FormSections'
import {
  buildIcs205DocxBlocks,
  buildIcs205ExportOptions,
  ics205ExportFilenameBase,
  type Ics205DocxBlock,
} from '@/features/ics205/export'
import type {
...
import { cloneIcs205FormState, getIcs205FormForExport } from '@/features/ics205/utils'
import {
...
type ExportBlock = Ics205DocxBlock
type ExportOptions = ReturnType<typeof buildIcs205ExportOptions>

type Ics205WorkspacePanelProps = {
  form: Ics205FormState | null
  setForm: React.Dispatch<React.SetStateAction<Ics205FormState | null>>
  ...
  onPatchSectionDraft: <S extends Ics205SectionId>(
    section: S,
...
export function Ics205WorkspacePanel({
  form,
  setForm,
  versions,
  canEdit,
  ...
  downloadDocx,
  downloadPdf,
}: Ics205WorkspacePanelProps) {
  const liveFormRef = useRef<Ics205FormState | null>(null)
  const [viewingPastVersion, setViewingPastVersion] = useState<Ics205Version | null>(null)
  ...
  const [previewBlocks, setPreviewBlocks] = useState<Ics205DocxBlock[]>([])
  ...
  const getExportForm = () => {
    if (!form) return null
    return getIcs205FormForExport(form, sectionDrafts)
  }
  ...
  const openPreview = () => {
    ...
    setPreviewBlocks(buildIcs205DocxBlocks(exportForm, getExportContext()))
  ...
  const exportWord = (blocks?: ExportBlock[]) => {
    ...
    downloadDocx(
      `ICS-205_${ics205ExportFilenameBase(exportForm)}_${stamp}.docx`,
      blocks ?? buildIcs205DocxBlocks(exportForm, context),
      buildIcs205ExportOptions(exportForm, context)
    )
  }
  ...
  const exportPdf = (blocks?: ExportBlock[]) => {
    ...
    const stamp = new Date().toISOString().slice(0, 16).replace(/[:T]/g, '-')
    downloadPdf(
      `ICS-205_${ics205ExportFilenameBase(exportForm)}_${stamp}.pdf`,
      blocks ?? buildIcs205DocxBlocks(exportForm, context),
      buildIcs205ExportOptions(exportForm, context)
    )
  }
  ...
  const viewVersion = (version: Ics205Version) => {
    if (!form) return
    if (!viewingPastVersion) {
      liveFormRef.current = cloneIcs205FormState(form)
    }
    setForm(cloneIcs205FormState(version.snapshot))
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-end gap-2">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              <DropdownMenuLabel>Export ICS-205</DropdownMenuLabel>
              <DropdownMenuSeparator />
        ...
        {isCreatingSignedVersion && !viewingPastVersion && (
          <div className="flex items-center justify-end rounded-md border border-sky-400 bg-sky-50/60 px-3 py-2 dark:border-sky-500 dark:bg-sky-500/10">
            <Button
              ...
              onClick={() => {
                setSignNameInput('You')
                setSignDialog({ mode: 'new-version', role: 'Communications Unit Leader' })
              }}
            >
      ...
      >
        <DialogContent className="!w-[60vw] !max-w-[60vw] sm:!max-w-[60vw]">
          <div className="flex items-center gap-2 px-1 pb-2 text-sm font-semibold">
            ...
            {versionDialogKind === 'signed' ? 'Signed versions' : 'Version history'} — ICS-205
          </div>
                ...
                {[...shownVersions].reverse().map((version) => {
                  ...
                  const versionLabel =
                    indexInAll === versions.length - 1 && versionDialogKind === 'all'
                      ...
                      : `v${indexInAll + 1}`
                  const preview = (
                    version.snapshot.radioChannels[0]?.channelNameTalkgroup ||
      ...
      <Ics205ExportPreviewDialog
        open={isPreviewOpen}
        onOpenChange={setIsPreviewOpen}
        title={`ICS-205 Preview — ${form.incidentName || incidentName || 'Incident Radio Communications Plan'}`}

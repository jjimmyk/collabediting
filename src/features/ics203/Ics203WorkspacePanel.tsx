...
import { Ics203ExportPreviewDialog } from '@/features/ics203/Ics203ExportPreviewDialog'
import { Ics203FormSections } from '@/features/ics203/Ics203FormSections'
import {
  buildIcs203DocxBlocks,
  buildIcs203ExportOptions,
  ics203ExportFilenameBase,
  type Ics203DocxBlock,
} from '@/features/ics203/export'
import type {
...
import { cloneIcs203FormState, getIcs203FormForExport } from '@/features/ics203/utils'
import { cn } from '@/lib/utils'

type ExportBlock = Ics203DocxBlock
type ExportOptions = ReturnType<typeof buildIcs203ExportOptions>

type Ics203WorkspacePanelProps = {
  form: Ics203FormState | null
  setForm: React.Dispatch<React.SetStateAction<Ics203FormState | null>>
  ...
  onPatchSectionDraft: <S extends Ics203SectionId>(
    section: S,
...
export function Ics203WorkspacePanel({
  form,
  setForm,
  versions,
  canEdit,
  ...
  const liveFormRef = useRef<Ics203FormState | null>(null)
  const [viewingPastVersion, setViewingPastVersion] = useState<Ics203Version | null>(null)
  ...
  const [previewBlocks, setPreviewBlocks] = useState<Ics203DocxBlock[]>([])
  ...
  const getExportForm = () => {
    if (!form) return null
    return getIcs203FormForExport(form, sectionDrafts)
  }
  ...
  const openPreview = () => {
    ...
    setPreviewBlocks(buildIcs203DocxBlocks(exportForm, getExportContext()))
  ...
  const exportWord = (blocks?: ExportBlock[]) => {
    ...
    downloadDocx(
      `ICS-203_${ics203ExportFilenameBase(exportForm)}_${stamp}.docx`,
      blocks ?? buildIcs203DocxBlocks(exportForm, context),
      buildIcs203ExportOptions(exportForm, context)
    )
  }
  ...
  const exportPdf = (blocks?: ExportBlock[]) => {
    ...
    const stamp = new Date().toISOString().slice(0, 16).replace(/[:T]/g, '-')
    downloadPdf(
      `ICS-203_${ics203ExportFilenameBase(exportForm)}_${stamp}.pdf`,
      blocks ?? buildIcs203DocxBlocks(exportForm, context),
      buildIcs203ExportOptions(exportForm, context)
    )
  }
  ...
  const viewVersion = (version: Ics203Version) => {
    if (!form) return
    if (!viewingPastVersion) {
      liveFormRef.current = cloneIcs203FormState(form)
    }
    setForm(cloneIcs203FormState(version.snapshot))
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-end gap-2">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              <DropdownMenuLabel>Export ICS-203</DropdownMenuLabel>
              <DropdownMenuSeparator />
      ...
      >
        <DialogContent className="!w-[60vw] !max-w-[60vw] sm:!max-w-[60vw]">
          <div className="flex items-center gap-2 px-1 pb-2 text-sm font-semibold">
            ...
            {versionDialogKind === 'signed' ? 'Signed versions' : 'Version history'} — ICS-203
          </div>
                ...
                {[...shownVersions].reverse().map((version) => {
                  ...
                  const versionLabel =
                    indexInAll === versions.length - 1 && versionDialogKind === 'all'
                      ...
                      : `v${indexInAll + 1}`
                  const preview = (
                    version.snapshot.operationsChief ||
                    version.snapshot.agencyRepresentatives[0]?.representativeName ||
      ...
      <Ics203ExportPreviewDialog
        open={isPreviewOpen}
        onOpenChange={setIsPreviewOpen}
        title={`ICS-203 Preview — ${form.incidentName || incidentName || 'Organization Assignment List'}`}
